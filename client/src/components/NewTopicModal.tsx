    ...
    <div className="fixed inset-0 z-50 bg-black/60 flex items-end sm:items-center justify-center">
      <div className="w-full sm:max-w-lg bg-white rounded-t-2xl sm:rounded-2xl fade-slide-up max-h-[90vh] flex flex-col">
        ...
        <div className="shrink-0 flex items-center justify-between px-5 py-4 border-b border-border">
          <h3 className="text-lg font-bold text-gray-900">Buat Group Baru</h3>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {canCreateL1 && (
            <div className="flex gap-2">
              <button onClick={() => setType("level1")}
                className={`flex-1 h-10 rounded-lg border text-sm font-medium transition ${type === "level1" ? "border-primary bg-primary/10 text-primary" : "border-gray-300 text-gray-600 hover:bg-gray-50"}`}>
                Group Baru
              </button>
              <button onClick={() => setType("sub")}
                className={`flex-1 h-10 rounded-lg border text-sm font-medium transition ${type === "sub" ? "border-primary bg-primary/10 text-primary" : "border-gray-300 text-gray-600 hover:bg-gray-50"}`}>
                Sub-group
              </button>
            </div>
          )}
          ...
          {type === "sub" && (
            <label className="flex flex-col gap-1.5">
              <span className="text-sm font-semibold text-gray-800">Group Parent</span>
              {parents.length === 0 ? (
                <p className="text-sm text-red-600">Belum ada group yang bisa dijadikan parent. Buat Group Baru terlebih dahulu.</p>
              ) : (
          ...
          {/* Description */}
          <label className="flex flex-col gap-1.5">
            <span className="text-sm font-semibold text-gray-800">Deskripsi <span className="font-normal text-gray-500">(opsional)</span></span>
          ...
          <div className="flex flex-col gap-1.5">
              ...
              {filteredUsers.length > 0 && (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (allSelected) {
                      setSelectedUsers((s) => s.filter((id) => !allIds.includes(id)));
                    } else {
                      setSelectedUsers((s) => [...new Set([...s, ...allIds])]);
                    }
                  }}
                  className="text-xs text-primary hover:underline shrink-0 ml-2"
                >
                  {filteredUsers.every((u) => selectedUsers.includes(u.id)) ? "Batal semua" : "Pilih semua"}
                </button>
              )}
            </div>
            ...
            <div className="max-h-44 overflow-y-auto border border-border rounded-lg divide-y divide-border">
              {filteredUsers.length === 0 ? (
                <div className="px-3 py-4 text-sm text-gray-500 text-center">Tidak ada user ditemukan</div>
              ) : filteredUsers.map((u) => {
                const selected = selectedUsers.includes(u.id);
                return (
                  <button key={u.id} onClick={() => toggleUser(u.id)}
                    className={`w-full flex items-center gap-3 px-3 py-2 text-left transition ${selected ? "bg-primary/5" : "hover:bg-gray-50"}`}>
                    <span className={`w-5 h-5 rounded border-2 flex items-center justify-center text-xs shrink-0 transition ${selected ? "bg-primary border-primary text-white" : "border-gray-300"}`}>
                      {selected && "✓"}
                    </span>
        ...
        <div className="shrink-0 px-5 py-4 border-t border-gray-200">
          ...
          <div className="flex gap-2">
            <button onClick={onClose} className="flex-1 h-10 rounded-lg border border-gray-300 text-sm text-gray-600 hover:bg-gray-50 transition">
              Batal
            </button>
